...
<!--MemberVo파일 import  자동으로 import를 위해 자동완성 습관중요-->
...
<body>
	...
	<div id="wrapper">
		...
		<!-- Header -->
		<header id="header" class="alt"> <a href="index.html" class="logo"><strong>Forty</strong> <span>by HTML5 UP</span>
		</a> 
		<nav>
		<!-- jstl 방식  --> <!-- 1.일반 회원 로그인시 ~  --> <!-- 2.admin계정으로 로그인시 회원괸리 메뉴을 띄우시요  -->

		<c:if test="${member != null}">

			<!-- 쌍안에 소따옴표  -->
			<c:if test="${member.email == 'admin' }">
				<a href="SelectAllService">회원괸리</a>
			</c:if>
			<c:if test="${member.email != 'admin' }">
